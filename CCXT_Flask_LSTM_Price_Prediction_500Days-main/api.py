...
def forecast(num_dates):
    try:
        predicted_values = predict_values_for_future_dates(model, last_results, start_date, int(num_dates) + 1,
                                                            time_steps)
        ...
        date_value_pairs.pop(next(iter(date_value_pairs)))  
        forecast_data = {
        ...
        forecast_data['graph'] = fig.to_json()
